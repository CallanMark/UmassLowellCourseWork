...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        # Longer than 2 lines but functionallity the same
        if kv is None:
            k = self.k(q)  # [batch_size, query_seq_len, hidden]
            v = self.v(q)  # [batch_size, query_seq_len, hidden]
            q = self.q(q)  # [batch_size, query_seq_len, hidden]
        else:
        ...
        if key_padding_mask is not None:
            ...
            if key_padding_mask is not None:
                scores = scores.view(bs, self.num_heads, attending_seq, attended_seq)  # [batch_size, num_heads, attending_seq, attended_seq]
                scores = scores.masked_fill(key_padding_mask[:, None, None, :].to(torch.bool), float("-inf"))  # [batch_size, num_heads, attending_seq, attended_seq]
                scores = scores.view(bs * self.num_heads, attending_seq, attended_seq)  # [batch_size * num_heads, attending_seq, attended_seq]    
                        # YOUR CODE ENDS HERE

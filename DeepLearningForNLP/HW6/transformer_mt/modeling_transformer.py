...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.self_attention = MultiHeadAttention(hidden, hidden, num_heads, causal=True)  # Causal for target sequence
        self.cross_attention = MultiHeadAttention(hidden, hidden, num_heads, causal=False)  # Non-causal for encoder states
        self.att_layer_norm = nn.LayerNorm(hidden)
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        # 2. Residual + 3. LayerNorm
        x = self.att_layer_norm(x + residual)  # [batch_size, query_seq_len, hidden]
        # 4. Cross-Attention
        residual = x  # [batch_size, query_seq_len, hidden]
        x = self.cross_attention(x, kv=encoder_hidden_states, key_padding_mask=key_padding_mask)  # [batch_size, query_seq_len, hidden]
        # 5. Residual + 6. LayerNorm
        x = self.cross_att_layer_norm(x + residual)  # [batch_size, query_seq_len, hidden]
        # 7. Fully-Connected Layer
        residual = x  # [batch_size, query_seq_len, hidden]
        x = self.fcn(x)  # [batch_size, query_seq_len, hidden]
        # 8. Dropout
        x = self.dropout(x)  # [batch_size, query_seq_len, hidden]
        # 9. Residual + 10. LayerNorm
        x = self.fcn_layer_norm(x + residual)  # [batch_size, query_seq_len, hidden]
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.src_embedding = nn.Embedding(src_vocab_size, hidden)
        self.tgt_embedding = nn.Embedding(tgt_vocab_size, hidden)
        ...
        # NOTE: Unsure about this review later !
        self.encoder_layers = nn.ModuleList()
        for _ in range(num_layers):
            self.encoder_layers.append(
                TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout, causal=False)
            )
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        x = self.src_embedding(input_ids) * (self.hidden ** 0.5)  # [batch_size, seq_len, hidden]
        x = self._add_positions(x)  # [batch_size, seq_len, hidden]
        x = self.dropout(x)  # [batch_size, seq_len, hidden]
        encoder_hidden_states = x
        for layer in self.encoder_layers:
            encoder_hidden_states = layer(encoder_hidden_states, key_padding_mask=key_padding_mask)  # [batch_size, seq_len, hidden]
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        x = self.tgt_embedding(decoder_input_ids) * (self.hidden ** 0.5)  # [batch_size, out_seq_len, hidden]
        x = self._add_positions(x)  # [batch_size, out_seq_len, hidden]
        x = self.dropout(x)  # [batch_size, out_seq_len, hidden]
        ...
        for layer in self.decoder_layers:
            ...
            )  # [batch_size, out_seq_len, hidden]
        logits = self.out_proj(decoder_hidden_states)  # [batch_size, out_seq_len, tgt_vocab_size]

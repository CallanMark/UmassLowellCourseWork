...
def affine_forward(x:np.ndarray, w:np.ndarray, b:np.ndarray):
    ...
    # Resape X before dot product computation 
    x_reshaped = x.reshape(x.shape[0],-1)@w +b 
    out = x_reshaped.dot(w) 
    out += b # Add bias to each row 
...
def svm_loss(x, y):
    ...
    margins = x - correct_class_scores[:, np.newaxis] + 1.0  
    margins[range(N), y] = 0  
    loss = np.sum(np.maximum(0, margins)) / N
    
    dx = np.zeros_like(x)  
    dx[margins > 0] = 1
    dx[range(N), y] = -np.sum(margins > 0, axis=1)
    dx /= N  

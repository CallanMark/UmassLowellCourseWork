...
"""Train a neural network classifier.
    Example usage (venv activated):
    python train.py --max_vocab_size 10000 --hidden_size 128 --use_batch_norm --dropout 0.5 --weight_decay 0.01 --lr 0.001 --batch_size 32 --max_epochs 5 --early_stopping 3 --device cuda --output_dir ./models , 
    conda active venv-hw3   
"""
...
def main(args):
    ...
    for _ in tqdm(range(args.max_epochs), desc="Epochs"):
        ...
        for x, y in dataloader:
            ...
            y = y.to(device).float()  
            class_probs = model(x)
            loss = F.binary_cross_entropy(class_probs, y.squeeze())  # Was getting an error and  squeeze() fixed it 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # YOUR CODE ENDS
        ...
        # Our implementation is 2 lines , Unusre if this correct 
        ...
        if epochs_without_improvement > args.early_stopping:
            logger.info("Training stopped early due to no improvement after " ,args.early_stopping ," epochs")
            break 

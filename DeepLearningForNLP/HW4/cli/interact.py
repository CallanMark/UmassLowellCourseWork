...
def preprocess_text(text, tokenizer):
    ...
    ids_tensor = torch.tensor(ids, dtype=torch.long)  # Convert to LongTensor
    count_vec = torch.zeros(tokenizer.get_vocab_size()).scatter_add_(0, ids_tensor, torch.ones_like(ids_tensor, dtype=torch.float))  # Create count vector
    count_vec.unsqueeze_(0)
...
def main(args):
    ...
    dropout_prob=0.0,  # Dropout set to 0 for inference
    use_batch_norm=train_args["use_batch_norm"]
)
    state_dict = torch.load(os.path.join(args.model_dir, "model_checkpoint.pt"))

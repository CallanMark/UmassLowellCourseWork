...
class FcnBinaryClassifier(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, dropout_prob=0.5, use_batch_norm=False):
        ...
        # YOUR CODE STARTS
        '''
        Input size = width of data , e.g X(n*m) , hidden_size = num_hidden_layers
        '''
    ...
    def forward(self, x):
        ...
        if self.batch_norm is True :  # Could use is not none here , but don't think it makes a difference 
            x = self.batch_norm(x)
        x = self.dropout(x)
